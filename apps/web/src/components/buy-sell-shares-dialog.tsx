...
export function BuySellSharesDialog({
  ...
  // Calculate preview
  const priceInWei = price ? parseUnits((Number(price) / 100).toString(), 18) : 0n;
  ...
  const handleBuy = async () => {
    ...
    writeContract({
      ...
      args: [flightId as `0x${string}`, outcomeType, positionValue, sharesToBuy, pricePerShare],
    });
  };
  ...
  const handleSell = async () => {
    ...
    writeContract({
      ...
      args: [flightId as `0x${string}`, outcomeType, positionValue, sharesToSell, pricePerShare],
    });
  };
  ...
  const potentialProfit = shares && price && costOrPayout && isBuying ? 
    potentialWinnings - (costOrPayout as bigint) : 0n;

  if (isSuccess) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={handleClose}>
        <Card className="w-full max-w-md border-white/10 bg-slate-900 text-white" onClick={(e) => e.stopPropagation()}>
          <CardHeader>
            <CardTitle className="text-green-400">✓ Transaction Complete!</CardTitle>
    ...
    return (
      <div className="flex gap-1">
        <Button
          ...
          className="h-7 px-3 text-xs bg-sky-500 text-white hover:bg-sky-600"
        >
          BUY
        </Button>
        <Button
          ...
          className="h-7 px-3 text-xs border-white/30 bg-transparent text-white hover:bg-white/10"
        >
          SELL
        </Button>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={handleClose}>
      <Card className="w-full max-w-md border-white/10 bg-slate-900 text-white" onClick={(e) => e.stopPropagation()}>
        <CardHeader>
          <CardTitle>{isBuying ? "Buy" : "Sell"} Shares</CardTitle>
          <CardDescription className="text-slate-300">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isBuying && userShares !== undefined && (
              <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                <p className="text-xs text-slate-400">Your {position} Shares</p>
                <p className="text-sm text-white">{formatUnits(userShares as bigint, 18).substring(0, 10)}</p>
              </div>
            )}
            ...
            {isBuying && costOrPayout !== undefined && shares && price && (
              <div className="space-y-2">
                <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3">
                  <p className="text-xs text-sky-300">Cost to buy {shares} {position} shares at {price}%</p>
                  <p className="text-lg font-semibold text-sky-200">
                    ${formatUnits(costOrPayout as bigint, 18).substring(0, 8)}
                  </p>
                </div>
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3">
                  <p className="text-xs text-emerald-300">If you win (each share pays $1)</p>
                  <p className="text-sm text-emerald-200">
                    Winnings: ${formatUnits(potentialWinnings, 18).substring(0, 8)}
                  </p>
                  <p className="text-sm font-semibold text-emerald-200">
                    Profit: ${formatUnits(potentialProfit, 18).substring(0, 8)}
                  </p>
                </div>
              </div>
            )}
            ...
            {!isBuying && costOrPayout !== undefined && shares && price && (
              <div className="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3">
                <p className="text-xs text-sky-300">You will receive</p>
                <p className="text-lg font-semibold text-sky-200">
                  ${formatUnits(costOrPayout as bigint, 18).substring(0, 8)}
                </p>
              </div>
